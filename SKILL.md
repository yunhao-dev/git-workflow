---
name: git-workflow
description: "通用 Git 提交、分支与发布约定（Claude Code 技能）：commit 前缀按 Commit 规范标注类型；分支模型为 main/release-xxx/dev/test/feature/hotfix；区分 main 推送、部署标记和正式版本 Tag，并支持四段修订版本，避免部署修复挤占 PATCH。多仓库场景各自独立 git、禁止混合提交。使用时：准备 git commit、切分支、合并、push、打 Tag 或发布时遵循此 skill。"
---
...
- 正式版本可按项目选择一种模式，并在同一项目中保持一致：
  - 标准三段：`vMAJOR.MINOR.PATCH`（例如 `v2.4.1`），适合严格遵循 SemVer 的项目。
  - 四段修订版：`vMAJOR.MINOR.PATCH.REVISION`（例如 `v2.4.1.12`）。前三段表示产品功能版本，第四段表示该版本对外产物的修订轮次，从 `0` 开始、按需递增，不占用 PATCH。
- 四段格式不是标准 SemVer。采用前先确认包管理器、发布平台和版本比较工具可以识别；若工具只接受 SemVer，保留三段版本并使用 `v2.4.1-rev.12`，或改用部署标记。
...
- 四段模式下，只有确实对外交付了新的修订产物时才增加 REVISION；普通提交、部署重试和未交付的修复不增加任何一段。由三段模式迁移时，把已有 `vX.Y.Z` 视作 `vX.Y.Z.0`，下一个修订从 `vX.Y.Z.1` 开始，不移动旧 Tag。
- 若部署系统必须由 Tag 触发，使用不参与版本计算的部署标记，例如 `deploy/prod/20260904-01`；版本计算、Release Note 和包发布流程只识别项目选定的正式版本格式。
- 部署标记也应保持不可变；需要再次部署时创建新的部署标记，禁止移动或覆盖已经推送的 Tag。若平台支持，优先用 commit SHA 或 Deployment 记录部署，不额外创建 Tag。
- 若正式版本 Tag 已经发布，之后的修复改变了对外产物或行为，再发布时：三段模式递增 PATCH，四段模式优先递增 REVISION；不要修改或复用已有正式版本 Tag。
...
7. 用户要求打 Tag 时，先确认用途：正式版本使用项目选定的三段或四段格式，仅触发/记录部署则使用部署标记；创建前检查远程同名 Tag 是否已存在。

## 六、正式版本 Tag 与发布说明

仅当用户明确确认发布新版本时执行：

1. **确认版本号**：Tag 与项目版本号及构建产物一致。先确认项目采用三段还是四段模式、此次是否发布版本，以及使用哪个版本号。
2. **查看上一个正式版本 Tag**：只在项目选定的正式版本格式中选择上一版本，并记录其哈希；忽略 `deploy/...` 部署标记。四段模式迁移时可把最后一个三段 Tag 作为 `.0` 基线。
